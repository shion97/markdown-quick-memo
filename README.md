...
ホットキーランチャーから起動したアプリはバックグラウンドで待機します。`Alt + F4` またはウィンドウの閉じるボタンでは編集中の内容を保持したまま非表示へ戻り、次回のホットキー操作で即座に復帰します。プロセスまで終了する場合は `Ctrl + Q` またはファイルメニューの「閉じる」を使用します。

## 入力支援

入力中のMarkdown構造に応じて、次の入力を自動補完します。補完は編集欄のMarkdown原文へ直接反映され、通常のUndo/Redoで取り消せます。

- リスト行でEnterを押すと、箇条書き・番号付きリスト・チェックリストを次行へ継続します。番号付きリストはMarkdown原文へ`1. `を追加します。チェックリストは未チェックの`[ ]`を作成します。リスト記号は入力中かどうかにかかわらず原文のまま表示し、画面幅によって一つの項目が折り返される場合、二行目以降は一行目の本文先頭へ揃えます。
...
- 番号付きリストをEnterで継続した場合も、追加されたMarkdown原文の `1. ` をそのまま表示します。
- `-`、`*`、`+` と番号付きリストの記号は、Enter・Backspace・カーソル移動・クリックの前後で切り替えず、Markdown原文のまま表示します。
