...
- `Ctrl + Q`は操作中のウィンドウだけを非表示にし、編集状態を保持する。ホットキーは最後に操作したウィンドウを表示する。
- ウィンドウの閉じるボタンと`Alt + F4`は全ウィンドウの未保存内容を確認して本体を完全終了する。ランチャーは残るため再度ホットキーから起動できる。
...
| 新規 | `Ctrl + N` |
| 開く | `Ctrl + O` |
| 新しいタブで新規 | `Ctrl + Shift + N` |
| 新しいタブで開く | `Ctrl + Shift + O` |
| タブを閉じる | `Ctrl + D` |
| タブ操作を開始 / 編集へ戻る | `Ctrl + K` |
| タブを上 / 下へ切り替える | タブ操作中に`Ctrl + ↑` / `Ctrl + ↓` |
| タブを狭める / 広げる | タブ操作中に`Ctrl + ←` / `Ctrl + →` |
| タブ操作から編集へ戻る | `Enter` / `Esc` |
| 保存 | `Ctrl + S` |
...
| 目次を広げる / 狭める | 目次操作中に`Ctrl + ←` / `Ctrl + →` |

## タブとウィンドウ

- 左側にタブ、右側に目次を表示します。タブと目次の幅は独立して240～480ピクセルの範囲を40ピクセルずつ変更できます。編集欄の幅を560px以上確保できない場合、タブは左端へのマウスオーバーや`Ctrl + K`で表示します。
- タブ上部のピンを外すと、タブ帯を細く格納した状態を次回起動時にも復元します。格納中の見出しとファイル名は折り返さず、選択中ファイルの枠は編集欄側を開けて表示します。
- 新しいタブは一番下へ追加します。末尾の`+`は新規タブ、各行の`×`はタブを閉じます。最後のタブを閉じると空の新規文書が残ります。
- `Ctrl + N`と`Ctrl + O`は現在のタブの文書を置き換えます。同じファイルを開く場合は、他のウィンドウも含めて既存のタブへ移動します。
- タブをドラッグして並べ替え、ウィンドウの枠外で離すと別ウィンドウへ分離します。別ウィンドウの左側のタブ一覧へドロップすると合流します。最後のタブは新しいウィンドウへ分離できませんが、既存ウィンドウへの合流は可能です。
- タブごとに取り消し履歴、選択位置、スクロール位置、閲覧モード、目次の折りたたみを保持します。移動先の受領確認後に元のタブを閉じます。ドラッグや受け渡し待ちは`Esc`で取り消せます。
- 非表示のタブも自動保存の対象です。アプリを完全終了した後のタブ構成の復元には対応していません。
