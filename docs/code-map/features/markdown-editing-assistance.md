...
- `markdown_quick_memo/app.py#MarkdownQuickMemoApp._create_list_marker_widget` — 余白のない固定幅Canvasへ右揃えして非アクティブ行のプレビューマーカーを表示する。
- `markdown_quick_memo/app.py#MarkdownQuickMemoApp._mount_decoration` — リストの埋め込みCanvasへ同じ余白タグを適用し、原文表示との実描画位置を揃える。
